...
class DashboardController extends Controller
{
    public function adminDashboard(Request $request)
    {
        try {
            ...
            $todayAbsent = max(0, $activeEmployees - $todayAttendance);
            ...
            $departmentStats = User::workers()
                ...
                ->map(function($users, $department) {
                    ...
                    $present = $users->filter(function($user) {
                        return $user->attendances && $user->attendances->count() > 0;
                    })->count();
            ...
            return response()->json([
                'overview' => [
                    ...
                    'monthly_hours' => round((float)($monthlyHours ?? 0), 2),
                ],
    ...
    public function workerDashboard(Request $request)
    {
        try {
            ...
            $monthlyStats = [
                ...
                'total_hours' => (float)($monthlyAttendance->sum('total_hours') ?? 0),
                'average_hours' => (float)($monthlyAttendance->avg('total_hours') ?? 0),
            ...
            for ($i = 6; $i >= 0; $i--) {
                ...
                $weeklyHours[] = [
                    'date' => $date->format('M d'),
                    'day' => $date->format('D'),
                    'hours' => $attendance ? (float)($attendance->total_hours ?? 0) : 0
                ];
            }
            ...
            return response()->json([
                ...
                'monthly_stats' => [
                    ...
                    'total_hours' => round((float)($monthlyStats['total_hours'] ?? 0), 2),
                    'average_hours' => round((float)($monthlyStats['average_hours'] ?? 0), 2),
            ...
            \Log::error('Worker dashboard error: ' . $e->getMessage(), ['trace' => $e->getTraceAsString()]);
            return response()->json([
